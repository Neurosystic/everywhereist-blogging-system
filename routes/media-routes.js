...
router.get("/likeArticle/:articleId", async function (req, res) {
  ...
  await likeArticleDao.registerArticleLiked(
    articleId,
    userId,
    getCurrentTime()
  );
...
router.get("/deleteComment/:commentId", async function (req, res) {
  ...
  if (
    res.locals.user.id == comment.commenter_id ||
    res.locals.user.id == article.author_id
  ) {
...
router.get("/subscribe/:authorId", async function (req, res) {
  ...
  await subscriptionDao.registerSubscription(
    subscriberId,
    authorId,
    date_published
  );
...
router.post(
  "/readNotification/:notificationId/:evokerId/:receiverId",
